...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        with KittiLoader(object_dir=os.path.join(dataset_dir, 'evaluation'), queue_size=100, require_shuffle=False, is_testset=False, batch_size=args.single_batch_size * cfg.GPU_USE_COUNT, use_multi_process_num=0, multi_gpu_sum=cfg.GPU_USE_COUNT) as test_loader:
            ...
            with tf.Session(config=config) as sess:
                ...
                while True:
                    ...
                    if data is None:
                        ...
                        break
                    if args.vis:
                        tags, results, front_images, bird_views, heatmaps = model.predict_step(sess, data, summary=False, vis=True)
                    else:
                        tags, results = model.predict_step(sess, data, summary=False, vis=False)
                    # ret: A, B
                    # A: (N) tag
